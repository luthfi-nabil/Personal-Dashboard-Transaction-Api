...
pub fn select_all_sources(conn: &mut PooledConn, source: &SourceV2) -> Result<Vec<SourceV2>> {
    let mut query = String::from(
        "SELECT source_id, source, created_date, created_by, is_active FROM source WHERE is_active = 1",
    );
    ...
    let result = conn.exec_map(
        ...
        |(source_id, source, created_date, created_by, is_active): (
            ...
            let source_id = Uuid::parse_str(&source_id).unwrap_or_else(|_| Uuid::nil());
