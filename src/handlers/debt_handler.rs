...
pub async fn post_debt_api(req: HttpRequest, debt: web::Json<Debt>) -> HttpResponse {
    ...
    if new_debt.debt_type == 1 {
        ...
        let _check_spending = crate::repository::spending_repository_v2::select_spendings(
            &mut conn,
            &spending_param,
            Some(created_by.clone()),
        );
        println!(
            "Debug: Check spending result {:?}",
            _check_spending.as_ref().unwrap().len() > 0
        );
...
pub async fn update_debt_status(req: HttpRequest, param: web::Json<DebtParam>) -> HttpResponse {
    ...
    if update_debt_param.debt_type == 1 {
        let spending_data = crate::repository::spending_repository_v2::select_spendings(
            &mut conn,
            &SpendingParam {
                description: None,
                spending_category: None,
        ...
        if spending_data.is_ok() && spending_data.as_ref().unwrap().len() > 0 {
            ...
            update_debt_param.debt_spending_id = param
                .debt_spending_id
                .as_ref()
                .and_then(|id| Uuid::parse_str(id).ok());
        }
    } else if update_debt_param.debt_type == 2 {
        let earning_data = crate::repository::earning_repository_v2::select_earnings(
            &mut conn,
            &EarningParam {
                description: None,
                earning_category: None,
        ...
        if earning_data.is_ok() && earning_data.as_ref().unwrap().len() > 0 {
            status_transaction_data = true;
            update_debt_param.debt_earning_id = param
                .debt_earning_id
                .as_ref()
                .and_then(|id| Uuid::parse_str(id).ok());
        }
